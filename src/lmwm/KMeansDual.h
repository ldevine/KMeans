...
namespace lmwm  {
	...
	class KMeansDual : public Clusterer<T> {
	public:
		...
		KMeansDual(int numClusters, int numThreads, vector<T*> &data_1, vector<T*> &data_2) :
			_numClusters(numClusters),
			_seeder(new SEEDER()),
	...
	private:
		 ...
		 */
		void cluster(vector<T*> &d, size_t clusters) {
			// Setup initial state.
			_iterCount = 0;
		...
		void vectorsToNearestCentroid(vector<T*> &data) {
			...
			auto func = [&](int i) {
				auto nearest = _optimizer.nearest(data2[i], _centroids);
