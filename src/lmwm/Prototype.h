...
#include "../lmw/BitMapList8.h"
#include "../lmw/BitMapList16.h"
...
namespace lmwm {
	...
	using std::vector;
	using lmw::BitMapList8;
	using lmw::BitMapList16;
	...
	struct meanBitPrototype {
		 ...
		 */
		void operator()(BitVector *t1, vector<BitVector*> &objs,
		vector<int> &weights) const {

			int bitCountPerDimension[4096];
			block_type *data = t1->getData();
			;
			size_t vecSize = t1->size();
			size_t dataSize = sizeof (data[0]) * 8;
			size_t numBlocks = t1->getNumBlocks();
	...
	struct meanBitPrototype2 {
		 ...
		 * We assume that the length of bit vectors is less than 65536 and greater than 0.
		 */
		void operator()(BitVector *t1, vector<BitVector*> &objs, vector<int> &weights) const {
	...
	struct meanBitPrototype8 {
		...
		void operator()(BitVector *t1, vector<BitVector*> &objs,
			...
			if (weights.size() != 0) {
				...
				halfCount = static_cast<int>(objs.size()) / 2;
			}
			//cout << "\n";
			for (size_t s = 0; s < t1->size(); s++) {
				if (bitCountPerDimension[s] > halfCount) t1->set(s);
